...
Token::Token() : type(T_EOF), lexeme(), line(1), col(1) {}
...
bool Tokenizer::eof() const { return _i >= _src.size(); }
...
bool Tokenizer::is_digit(char c) const { return (c >= '0' && c <= '9'); }
...
Token Tokenizer::read_number() {
  ...
  return Token(Token::T_NUMBER, _src.substr(start, _i - start), start_line,
               start_col);
}
...
Token Tokenizer::read_ident() {
  ...
  return Token(Token::T_IDENT, _src.substr(start, _i - start), start_line,
               start_col);
}
...
Token Tokenizer::next() {
  ...
  if (c == '{') {
    get();
    return Token(Token::T_LBRACE, "{", ln, cl);
  }
  if (c == '}') {
    get();
    return Token(Token::T_RBRACE, "}", ln, cl);
  }
  if (c == '(') {
    get();
    return Token(Token::T_LPAREN, "(", ln, cl);
  }
  if (c == ')') {
    get();
    return Token(Token::T_RPAREN, ")", ln, cl);
  }
  if (c == ';') {
    get();
    return Token(Token::T_SEMI, ";", ln, cl);
  }
  if (c == ',') {
    get();
    return Token(Token::T_COMMA, ",", ln, cl);
  }
