...
void BootStrap::_bind_servers_for_port(
    int port, const std::vector<ServerConfig> &servers) {
  ...
  for (size_t i = 0; i < servers.size(); ++i) {
    ...
    try {
      ...
      _logger << "[BootStrap] Listening on http:" << host << ":" << port
              << " (fd: " << sock_fd << ")" << std::endl;
    } catch (const std::exception &e) {
      std::cerr << "Error: Could not bind port " << port << " on host " << host
                << ": " << e.what() << std::endl;
      _logger << "[BootStrap] Failed to bind port " << port << " on host "
