 ...
 *
 * Cette classe utilise poll() pour surveiller à la fois les sockets (nouvelles
 * connexions) et les sockets clients (données entrantes) Elle distribue les
 * événements aux gestionnaires appropriés (handle, accept, read, close)
 */
class EventLoop {
public:
...
private:
  ...
  std::map<int, webserv::core::Client *> _cgiFds;
  ...
  // std::string _runGetMethod(const http::Request &req, const ServerConfig
  // &srvConfig); std::string _runDeletMethod(const http::Request &req, const
  // ServerConfig &srvConfig); std::string _runPostMethod(const http::Request
  // &req, const ServerConfig &srvConfig); std::string _runErrorMethod(const
  // http::Request &req, const ServerConfig &srvConfig);
  ...
  // std::string _generateErrorResponse(int code, const std::string& msg, const
  // ServerConfig& config);
};
} // namespace core
} // namespace webserv
